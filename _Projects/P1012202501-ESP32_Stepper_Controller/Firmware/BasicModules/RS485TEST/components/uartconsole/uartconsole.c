...
/*Define prototypes*/
void uartconsole_init(uint8_t UART_RX_PIN, uint8_t UART_TX_PIN, uint32_t BAUD_RATE, uint8_t DIR_CONTROL, char* delimiter);
...
void uartconsole_init(uint8_t UART_RX_PIN, uint8_t UART_TX_PIN, uint32_t BAUD_RATE, uint8_t DIR_CONTROL, char* delimiter)
{
    ...
    gpio_reset_pin(DIR_CONTROL);
    gpio_set_direction(DIR_CONTROL, GPIO_MODE_OUTPUT);
    gpio_set_level(DIR_CONTROL, 0);
...
void uartconsole_send(const char* data)
{
    ...
    //Set DIR_CONTROL pin to proper level
    gpio_set_level(DIR_CONTROL, 1);
    vTaskDelay(125 / portTICK_PERIOD_MS);

    // Send data over UART
    uart_write_bytes(UART_NUM_1, data, len);

    //Reset DIR_CONTROL back to listen state
    vTaskDelay(125 / portTICK_PERIOD_MS);
    gpio_set_level(DIR_CONTROL, 0);
}
